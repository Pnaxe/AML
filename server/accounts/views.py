...
class CustomerViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['post'])
    def move_to_screening(self, request, pk=None):
        """
        Mark customer as moved to screening queue.
        """
        customer = self.get_object()

        # Ensure customer remains unverified while in screening queue
        customer.kyc_verified = False
        customer.save(update_fields=['kyc_verified', 'updated_at'])
        ...
        if KYC_AVAILABLE:
            try:
                ...
                if kyc_profile.verification_status != 'IN_PROGRESS':
                    kyc_profile.verification_status = 'IN_PROGRESS'
                    kyc_profile.save(update_fields=['verification_status', 'updated_at'])
            except Exception:
                # Continue even if KYC profile sync fails.
                pass

        return Response({
            'message': 'Customer moved to screening queue',
            'customer_id': customer.id,
