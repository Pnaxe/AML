...
class ScreeningService:
    ...
    def screen_name(self, name: str, entity_type: str = 'INDIVIDUAL', country: str = '', email: str = '') -> Dict:
        """
        Perform manual screening for an ad hoc subject name.
        """
        subject = SimpleNamespace(
            customer_type='CORPORATE' if entity_type == 'ORGANIZATION' else 'INDIVIDUAL',
            first_name=name if entity_type != 'ORGANIZATION' else '',
            last_name='',
            company_name=name if entity_type == 'ORGANIZATION' else '',
            date_of_birth=None,
            registration_number='',
            country=country,
            email=email,
        )
        return self.screen_customer(subject)
